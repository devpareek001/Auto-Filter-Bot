...
import pyrogram
from info import *
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, InputMediaPhoto, InputMediaAnimation, WebAppInfo
...
async def give_filter(client, message):
    ...
    if maintenance_mode and message.from_user.id not in ADMINS:
        ...
        return
    await devdb.update_top_messages(message.from_user.id, message.text)
...
async def pm_text(bot, message):
    ...
    if content.startswith(("/", "#")):
        return  
    try:
        await devdb.update_top_messages(user_id, content)
            ...
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"{dev_size(file.file_size)}| {extract_tag(file.file_name)} {clean_filename(file.file_name)}", callback_data=f'file#{file.file_id}'
                    ),
                ]
                for file in files
            ]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            dev_msg = await client.send_cached_media(
                chat_id=BIN_CHANNEL,
                file_id=file_id,
            )
            fileName = {quote_plus(get_name(dev_msg))}
            dev_stream = f"{URL}watch/{str(dev_msg.id)}/{quote_plus(get_name(dev_msg))}?hash={get_hash(dev_msg)}"
            dev_download = f"{URL}{str(dev_msg.id)}/{quote_plus(get_name(dev_msg))}?hash={get_hash(dev_msg)}"
            btn= [[
                InlineKeyboardButton("𝖲𝗍𝗋𝖾𝖺𝗆", url=dev_stream),
                InlineKeyboardButton("𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽", url=dev_download)        
	    ]]
            await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup(btn)
	    )
            await dev_msg.reply_text(
        ...
        buttons = [[
            InlineKeyboardButton('🤵 Owner Info', url=OWNER_LNK),
            InlineKeyboardButton('🔐 Source Code', callback_data='source')
        ],[
            InlineKeyboardButton('🏠 Home', callback_data='start'),
            InlineKeyboardButton('❌ Close', callback_data='close_data')
        ],[
            InlineKeyboardButton('❗ Disclaimer ❗', callback_data='about_disclaimer')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    elif query.data == "about_disclaimer":
        await query.answer(
            "This bot does not host any files on its own server. All content is provided by users/third parties and we are not responsible for it.",
            show_alert=True
        )
        ...
        dev = await client.get_chat(int(grp_id))
        await query.message.edit(text=f"ᴄʜᴀɴɢᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ ꜱᴇᴛᴛɪɴɢꜱ ✅\nɢʀᴏᴜᴘ ɴᴀᴍᴇ - '{dev.title}'</b>⚙", reply_markup=InlineKeyboardMarkup(btn))
        ...
        if not await is_check_admin(client, int(grp_id), user_id):
            return await query.answer("<b>ɴᴇᴇᴅ ᴛᴏ ʙᴇ ᴀᴅᴍɪɴ ᴛᴏ ᴜꜱᴇ ᴛʜɪꜱ ✅.</b>", show_alert=True)    
        current_status = str(status).strip().lower() in ("true", "1", "yes", "on")
        new_status = not current_status
        await save_group_settings(int(grp_id), set_type, new_status)
        verify_status = new_status
        await query.answer("✅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛᴜʀɴᴇᴅ ᴏɴ" if verify_status else "❌ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛᴜʀɴᴇᴅ ᴏꜰꜰ")
        btn = [[
            InlineKeyboardButton(f'ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ: {"ᴏɴ" if verify_status else "ᴏꜰꜰ"}', callback_data=f'toggleverify#is_verify#{verify_status}#{grp_id}'),
	],[
            InlineKeyboardButton('ꜱʜᴏʀᴛɴᴇʀ', callback_data=f'changeshortner#{grp_id}'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if settings.get('button'):
            cap =f"<b>📂 Rᴇsᴜʟᴛs Fᴏʀ, {message.from_user.mention}</b>\n\n"
        else:
            cap =f"<b>📂 Rᴇsᴜʟᴛs Fᴏʀ, {message.from_user.mention}</b>\n\n"            
            for file_num, file in enumerate(files, start=1):
    ...
    if imdb and imdb.get('poster'):
        try:
            hehe = await message.reply_photo(photo=imdb.get('poster'), caption=cap, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML, reply_to_message_id=message.id)
            try:
            ...
            hmm = await message.reply_photo(photo=poster, caption=cap, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML, reply_to_message_id=message.id)
            try:
               if settings['auto_delete']:
                    await asyncio.sleep(DELETE_TIME)
                    await hmm.delete()
            ...
            logger.exception(e)
            fek = await message.reply_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML, reply_to_message_id=message.id)
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(DELETE_TIME)
                    await fek.delete()
        ...
        fuk = await message.reply_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_to_message_id=message.id)
        try:
            if settings['auto_delete']:
                await asyncio.sleep(DELETE_TIME)
                await fuk.delete()
